...
const ReportsPage = () => {
  ...
  console.log("reports in ReportsPage:", reports);
  ...
  // 긴급도별 통계 계산
  const urgencyStats = {
    urgent: reports.filter((report) => report.urgency === UrgencyLevel_e.Urgent)
      .length,
    normal: reports.filter((report) => report.urgency === UrgencyLevel_e.Normal)
      .length,
    low: reports.filter((report) => report.urgency === UrgencyLevel_e.Low)
      .length,
  };
  ...
  const filteredReports =
    selectedUrgency === "all"
      ? reports
      : reports.filter((report) => report.urgency === selectedUrgency);
    ...
    <div className="flex min-h-screen flex-col bg-gray-50">
      ...
      <div className="relative mx-auto w-full max-w-4xl flex-1 space-y-6 px-4 py-6">
            ...
            <div className="mb-20 overflow-hidden rounded-xl border bg-white shadow-sm">
              ...
              {filteredReports.length === 0 ? (
                <div className="p-8 text-center">
                  ...
                  >
                    <Plus className="h-4 w-4" />첫 신고 작성하기
                  </button>
                </div>
              ) : (
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-start gap-4">
                            <div
                              className={`${config.bgColor} flex-shrink-0 rounded-lg p-2`}
                            >
                              <Icon
                                className="h-5 w-5"
                                style={{ color: config.color }}
        ...
        <div className="fixed right-4 bottom-4 left-4">
          <button
            onClick={() => router.push("/reports/new")}
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-blue-500 px-6 py-4 font-semibold text-white shadow-lg transition-all duration-200 hover:bg-blue-600 hover:shadow-xl"
          >
